...
*/
	...
	//ini method untuk menarik seluruh data penghargaan
	...
	public function get_all_penghargaan_detail_json($personnel_id){
 	
	$query = $this->db->query("select a.*,b.name as tipe, date_format(start_date,'%d-%m-%Y') as tglmulai,date_format(end_date,'%d-%m-%Y') as tglakhir from human_pa_md_emp_recognition a left join human_cust_personnel_recognition_type b on b.recognition_type = a.recognition_type where a.personnel_id = '$personnel_id' order by start_date desc");
	        ...
	        foreach ($query->result() as $row) {
	            $list_data[] = array(
	                ...
	                'opsi'=> '<a href="'.base_url('penghargaan/penghargaan_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'" title="Update"> </a> <a href="'.base_url('penghargaan/penghargaan_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'" title="Hapus"></a> '
                        //'opsidelete'=> '<a href="'.base_url('penghargaan/penghargaan_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'"> </a>',
	                //'opsiupdate'=> '<a href="'.base_url('penghargaan/penghargaan_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'"></a>'
	            );
	        }
	     return $list_data;
	...
	public function penghargaan_detail_delete($personnel_id,$start_date,$end_date){
		$sql = $this->db->query("delete from human_pa_md_emp_recognition where personnel_id = '$personnel_id' AND start_date = '$start_date' AND end_date = '$end_date'");
		return $sql;
	}
	...
	//ini method untuk menyimpan data penghargaan baru
	public function pro_add_penghargaan($nik,$stor_tanggal_masuk,$nama){
	...
	return $sql;
         ...
         public function opt_tipe_penghargaan(){
	$sql = $this->db->query("select * from human_cust_personnel_recognition_type order by recognition_type asc");
	return $sql;
	}
	...
	public function penghargaan_detail_pro_add($data){
                //get last max end_date for update
                $sqla = $this->db->query ("select * from human_pa_md_emp_recognition where personnel_id = '$data[personnel_id]' and end_date = (select MAX(end_date) from human_pa_md_emp_recognition WHERE personnel_id = '$data[personnel_id]')");
                $personnel_idfirst = '';
                $start_datefirst = '';
...
personnel_id,start_date,end_date,status_process,recognition_type,reference_no,description,authority_officer,facility,recognition_currency,recognition_value,remark
 
                 */
		$sqlb = $this->db->query("insert into human_pa_md_emp_recognition (personnel_id,start_date,end_date,status_process,recognition_type,reference_no,description,authority_officer,facility,recognition_value,remark) VALUES ('$data[personnel_id]','$data[start_date]','$data[end_date]','$data[status_process]','$data[recognition_type]','$data[reference_no]','$data[description]','$data[authority_officer]','$data[facility]','$data[recognition_value]','$data[remark]')");
                ...
                $sqlc =$this->db->query("update human_pa_md_emp_recognition set end_date = '$data[start_date]' where personnel_id = '$personnel_idfirst' AND start_date = '$start_datefirst'");
	...
	public function penghargaan_detail_update($personnel_id,$start_date,$end_date){ 
            
		$sql = $this->db->query("select *,date_format(start_date,'%d-%m-%Y') as tanggalstart,date_format(end_date,'%d-%m-%Y') as tanggalend from human_pa_md_emp_recognition where personnel_id = '$personnel_id' and start_date = '$start_date' and end_date = '$end_date'");
		return $sql;
	}

	public function penghargaan_update_selected($id){
	...
	public function penghargaan_detail_pro_update($data){
		/*
                 * $data = array('personnel_id'=>$this->input->post('personnel_id'),
                        'start_date'=>$tanggalmulai,
                        'end_date'=>$tanggalakhir,
                        'status_process'=>$this->input->post('status_process'),
                        'recognition_type'=>$this->input->post('recognition_type'),
                        'reference_no'=>$this->input->post('reference_no'),
                        'description'=>$this->input->post('description'),
                        'authority_officer'=>$this->input->post('authority_officer'),
                        'facility'=>$this->input->post('facility'),
                      
                        'recognition_value'=>$this->input->post('recognition_value'),
                        'remark'=>$this->input->post('remark')
                        
                        );
                  */
		$query = $this->db->query("update human_pa_md_emp_recognition set status_process = '$data[status_process]', 
							recognition_type = '$data[recognition_type]',
							reference_no = '$data[reference_no]',
							description = '$data[description]',
							authority_officer = '$data[authority_officer]',
							facility = '$data[facility]',
							recognition_value = '$data[recognition_value]',
							remark = '$data[remark]'                                                       
	...
	public function penghargaan_delete($id){
		$query = $this->db->query("delete from human_pa_md_emp_personal where personnel_id = '$id' ");
		return $query;
	}

	public function pro_update_penghargaan($nik,$stor_tanggal_masuk,$nama){
	...
	public function penghargaan_update_pro_selected($data){
