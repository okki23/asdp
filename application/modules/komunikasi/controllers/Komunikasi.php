...
*/
	...
	public function __construct(){
		parent ::__construct();
		//panggil model komunikasi jika memang controller butuh transaksi data
		$this->load->model('model_komunikasi');
 	...
 	public function get_all_komunikasi_detail_json(){
		$personnel_id = $this->uri->segment(3);
		 
		//echo json_encode($this->model_komunikasi->get_all_komunikasi_detail_json($personnel_id));
		$data = json_encode($this->model_komunikasi->get_all_komunikasi_detail_json($personnel_id));
		echo "{\"records\":" . $data . "}";
	}

	public function get_all_pic_komunikasi_json(){
		$personnel_id = $this->uri->segment(3);
		 
		echo json_encode($this->model_komunikasi->get_all_pic_komunikasi_json($personnel_id));
	}
	...
	public function index()	{
		...
		$this->load->view('komunikasi/komunikasi_detail_view',$data);
		 

		 
	}

	public function komunikasi_detail_json(){
	...
	public function komunikasi_detail(){
		$personnel_id = $this->uri->segment(3);
	 	...
	 	//$this->model_komunikasi->komunikasi_update($id);
	 	$data_employee = $this->model_komunikasi->komunikasi_detail($personnel_id);
	...
	public function komunikasi_detail_delete($personnel_id,$start_date,$end_date){
		...
		$sqldelete = $this->model_komunikasi->komunikasi_detail_delete($personnel_id,$start_date,$end_date);
	 
		redirect(base_url('komunikasi/komunikasi_detail/'.$personnel_id));
	}

	public function komunikasi_detail_add(){
		$personnel_id = $this->uri->segment(3);
                $location = $this->uri->segment(1);
		...
		$sql        =   $this->db->query("SELECT *,substr(birth_date,1,4) as thnlahir FROM human_pa_md_emp_personal where personnel_id = '$personnel_id' order by start_date desc limit 1")->row_array();
	 	...
	 	$data = array('judul'=>'Human Resource Information System (HRIS) ASDP',
					  'error'=>$error,
                                          'location'=>$location,
					  'personnel_id'=>$personnel_id,
                                          'opt_tipe_komunikasi'=>$this->model_komunikasi->opt_tipe_komunikasi(),
	 	...
	 	$this->load->view('komunikasi/komunikasi_detail_add',$data);
	}
	...
	public function komunikasi_detail_update($personnel_id,$start_date,$end_date){
		...
		$sql = $this->db->query("SELECT *,substr(birth_date,1,4) as thnlahir FROM human_pa_md_emp_personal where personnel_id = '$personnel_id' order by start_date desc limit 1")->row_array();
 	 	...
 	 	$data_com = $this->model_komunikasi->komunikasi_detail_update($personnel_id,$start_date,$end_date);
	 	$data = array('judul'=>'Human Resource Information System (HRIS) ASDP',
					  'location'=>$location,
                                          'personnel_id'=>$personnel_id,
                                          'opt_tipe_komunikasi'=>$this->model_komunikasi->opt_tipe_komunikasi(),
	 	...
	 	$this->load->view('komunikasi/komunikasi_detail_update',$data);
	}

	public function cek_opt(){
		$sql = $this->model_komunikasi->opt_assignment_reason();
	...
	public function komunikasi_detail_pro_update(){
		...
		$sql = $this->model_komunikasi->komunikasi_detail_pro_update($data);

	 	 echo "<script language=javascript>
				alert('Perubahan Data Berhasil');
				window.location='".base_url()."/komunikasi/komunikasi_detail/".$data['personnel_id']."';
		      </script>";
                
		//redirect(base_url('komunikasi/komunikasi_detail/'.$data['personnel_id']));
	}
	 
	public function komunikasi_detail_pro_add(){
                $personnel_id = $this->input->post('personnel_id');
		...
		$sql = $this->model_komunikasi->komunikasi_detail_pro_add($data);
	...
	public function komunikasi_update_pro_selected(){
		...
		$sqlupdate = $this->model_komunikasi->komunikasi_update_pro_selected($data);
