...
class Model_komunikasi extends CI_Model {
/*
...
*/
	...
	//ini method untuk menarik seluruh data komunikasi
	...
	public function get_all_komunikasi_detail_json($personnel_id){
 	
	$query = $this->db->query("select a.*,b.name as com_type, date_format(start_date,'%d-%m-%Y') as tglmulai,date_format(end_date,'%d-%m-%Y') as tglakhir from human_pa_md_emp_communication a left join human_cust_personnel_communication_type b on b.communication_type = a.communication_type where a.personnel_id = '$personnel_id' order by start_date desc");
			...
			$list_data = null;
	                ...
	                'com_type'=>$row->communication_type,
                        'com_id'=>$row->communication_id,
                        'opsi'=> '<a href="'.base_url('komunikasi/komunikasi_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'" title="Update"> </a><a href="'.base_url('komunikasi/komunikasi_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'" title="Hapus"></a>'
                        //'opsidelete'=> '<a href="'.base_url('komunikasi/komunikasi_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'"> </a>',
	                //'opsiupdate'=> '<a href="'.base_url('komunikasi/komunikasi_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'"></a>'
	            );
	        }
	     return $list_data;
	}

	public function komunikasi_detail_delete($personnel_id,$start_date,$end_date){
		$sql = $this->db->query("delete from human_pa_md_emp_communication where personnel_id = '$personnel_id' AND start_date = '$start_date' AND end_date = '$end_date'");
		return $sql;
	}
	public function get_all_pic_komunikasi_json($personnel_id){
	...
	$query = $this->db->query("select *,case when gender = '1' then 'Pria' else 'Wanita' end as genderstatus, date_format(start_date,'%d %M %Y') as tanggalmasuknya from human_pa_md_emp_personal where personnel_id = '$personnel_id' order by personnel_id asc");
	        ...
	        foreach ($query->result() as $row) {
	            $list_data[] = array(
	                ...
	                'tipe_komunikasi'=>$row->tanggalmasuknya,
	                'alasan_komunikasi' => $row->tanggalmasuknya
	            );
	        }
	     return $list_data;
	}
	...
	//ini method untuk menyimpan data komunikasi baru
	public function pro_add_komunikasi($nik,$stor_tanggal_masuk,$nama){
	...
	$sql = $this->db->query("select * from human_pa_md_emp_communication order by training_category asc");
	return $sql;
	}
        
        public function opt_tipe_komunikasi(){
	$sql = $this->db->query("select * from human_cust_personnel_communication_type order by communication_type asc");
	return $sql;
	}
	...
	public function komunikasi_detail_pro_add($data){
                //get last max end_date for update
                $sqla = $this->db->query ("select * from human_pa_md_emp_communication where personnel_id = '$data[personnel_id]' and end_date = (select MAX(end_date) from human_pa_md_emp_communication WHERE personnel_id = '$data[personnel_id]')");
                $personnel_idfirst = '';
                $start_datefirst = '';
                ...
                //store new data
		$sqlb = $this->db->query("insert into human_pa_md_emp_communication (personnel_id,start_date,end_date,status_process,communication_type,communication_id,phone_country,phone_area,phone_no,phone_extension) VALUES ('$data[personnel_id]','$data[start_date]','$data[end_date]','$data[status_process]','$data[communication_type]','$data[communication_id]','$data[phone_country]','$data[phone_area]','$data[phone_no]','$data[phone_extension]')");
                ...
                $sqlc =$this->db->query("update human_pa_md_emp_communication set end_date = '$data[start_date]' where personnel_id = '$personnel_idfirst' AND start_date = '$start_datefirst'");
	...
	public function komunikasi_detail_update($personnel_id,$start_date,$end_date){ 
            
		$sql = $this->db->query("select *,date_format(start_date,'%d-%m-%Y') as tanggalstart,date_format(end_date,'%d-%m-%Y') as tanggalend from human_pa_md_emp_communication where personnel_id = '$personnel_id' and start_date = '$start_date' and end_date = '$end_date'");
		return $sql;
	}

	public function komunikasi_update_selected($id){
	...
	public function komunikasi_detail_pro_update($data){
 
		$query = $this->db->query("update human_pa_md_emp_communication set status_process = '$data[status_process]', 
	...
	public function komunikasi_delete($id){
		$query = $this->db->query("delete from human_pa_md_emp_personal where personnel_id = '$id' ");
		return $query;
	}

	public function pro_update_komunikasi($nik,$stor_tanggal_masuk,$nama){
	...
	public function komunikasi_update_pro_selected($data){
