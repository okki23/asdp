...
*/
	...
	public function __construct(){
		parent ::__construct();
		//panggil model hukuman jika memang controller butuh transaksi data
		$this->load->model('model_hukuman');
 	...
 	public function get_all_hukuman_detail_json(){
		$personnel_id = $this->uri->segment(3);
		 
		//echo json_encode($this->model_hukuman->get_all_hukuman_detail_json($personnel_id));
		$data = json_encode($this->model_hukuman->get_all_hukuman_detail_json($personnel_id));
		echo "{\"records\":" . $data . "}";
	}

	public function get_all_pic_hukuman_json(){
		$personnel_id = $this->uri->segment(3);
		 
		echo json_encode($this->model_hukuman->get_all_pic_hukuman_json($personnel_id));
	}
	...
	public function index()	{
		...
		$this->load->view('hukuman/hukuman_detail_view',$data);
		 

		 
	}

	public function hukuman_detail_json(){
	...
	public function hukuman_detail(){
		$personnel_id = $this->uri->segment(3);
	 	...
	 	//$this->model_hukuman->hukuman_update($id);
	 	$data_employee = $this->model_hukuman->hukuman_detail($personnel_id);
	...
	public function hukuman_detail_delete($personnel_id,$start_date,$end_date){
		...
		$sqldelete = $this->model_hukuman->hukuman_detail_delete($personnel_id,$start_date,$end_date);
                /*
                echo "<script language=javascript>
				alert('Penghapusan Data Berhasil');
				window.location='".base_url()."/hukuman/hukuman_detail/".$personnel_id."';
		      </script>";  
                 
                 */ 
		redirect(base_url('hukuman/hukuman_detail/'.$personnel_id));
	}

	public function hukuman_detail_add(){
		$personnel_id = $this->uri->segment(3);
                $location = $this->uri->segment(1);
		...
		$sql        =   $this->db->query("SELECT *,substr(birth_date,1,4) as thnlahir FROM human_pa_md_emp_personal where personnel_id = '$personnel_id' order by start_date desc limit 1")->row_array();
	 	...
	 	$data = array('judul'=>'Human Resource Information System (HRIS) ASDP',
					  'error'=>$error,
                                          'location'=>$location,
					  'personnel_id'=>$personnel_id,
                                          'opt_tipe_hukuman'=>$this->model_hukuman->opt_tipe_hukuman(),
					  'opt_doc_type'=>$this->model_hukuman->opt_doc_type(),
                                          'opt_country'=>$this->model_hukuman->opt_country(),
	 	...
	 	$this->load->view('hukuman/hukuman_detail_add',$data);
	}
	...
	public function hukuman_detail_update($personnel_id,$start_date,$end_date){
		...
		$sql = $this->db->query("SELECT *,substr(birth_date,1,4) as thnlahir FROM human_pa_md_emp_personal where personnel_id = '$personnel_id' order by start_date desc limit 1")->row_array();
 	 	...
 	 	$data_sanction = $this->model_hukuman->hukuman_detail_update($personnel_id,$start_date,$end_date);
	 	$data = array('judul'=>'Human Resource Information System (HRIS) ASDP',
					  'location'=>$location,
                                          'personnel_id'=>$personnel_id,
                                          'opt_tipe_hukuman'=>$this->model_hukuman->opt_tipe_hukuman(),
	 	...
	 	$this->load->view('hukuman/hukuman_detail_update',$data);
	}

	public function cek_opt(){
		$sql = $this->model_hukuman->opt_assignment_reason();
	...
	public function hukuman_detail_pro_update(){
		...
		$sql = $this->model_hukuman->hukuman_detail_pro_update($data);

	 	  echo "<script language=javascript>
				alert('Perubahan Data Berhasil');
				window.location='".base_url()."/hukuman/hukuman_detail/".$data['personnel_id']."';
		      </script>";
		//redirect(base_url('hukuman/hukuman_detail/'.$data['personnel_id']));
	}
	 
	public function hukuman_detail_pro_add(){
                $personnel_id = $this->input->post('personnel_id');
...
remark
		...
		$sql = $this->model_hukuman->hukuman_detail_pro_add($data);
	...
	public function hukuman_update_pro_selected(){
		...
		$sqlupdate = $this->model_hukuman->hukuman_update_pro_selected($data);
