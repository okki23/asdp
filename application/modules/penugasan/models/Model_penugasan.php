...
*/
	...
	$query = $this->db->query("select a.*,b.name as tipe_penugasan,c.name as alasan_penugasan,  date_format(a.start_date,'%d-%m-%Y') as tanggalmasuknya,date_format(a.end_date,'%d-%m-%Y') as tanggalberakhirnya from human_pa_md_emp_assignment_additional a
	        ...
	        foreach ($query->result() as $row) {
	            $list_data[] = array(
	                ...
	                'alasan_penugasan' => $row->alasan_penugasan,
                        'opsi'=> '<a href="'.base_url('penugasan/penugasan_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'" title="Update"> </a><a  onclick="return confirm(Anda Yakin?);"  href="'.base_url('penugasan/penugasan_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'" title="Hapus"></a>'
	...
	public function penugasan_detail_delete($personnel_id,$star_date,$end_date){
		$sql = $this->db->query("delete from human_pa_md_emp_assignment_additional where personnel_id = '$personnel_id' AND start_date = '$star_date' AND end_date = '$end_date'");
		return $sql;
	}
	...
	$query = $this->db->query("select *,case when gender = '1' then 'Pria' else 'Wanita' end as genderstatus, date_format(start_date,'%d %M %Y') as tanggalmasuknya from human_pa_md_emp_personal where personnel_id = '$personnel_id' order by personnel_id asc");
        ...
        public function penugasan_opt_posisi($lit_code_position){
 	$sql = $this->db->query("select a.*,b.nama_jabatan as namajabatan,c.nm_unit as namaunit,case when a.tipe = 'L' then 'Laut' else 'Darat' end as statuswilayah from lit_tab_posisi a 
LEFT JOIN lit_tab_jabatan b on b.id = a.id_jabatan
...
where a.lit_code_position = '$lit_code_position'");
	...
	$query = $this->db->query("select *, date_format(start_date,'%d %M %Y') as tglmulai, date_format(end_date,'%d %M %Y') as tglakhir from human_pa_md_emp_assignment_additional where personnel_id = '$personnel_id'");
	...
	public function penugasan_detail($id){
		$query = $this->db->query("select a.*,b.name as tipeasignment,c.name as tipereason from human_pa_md_emp_assignment_additional a
			left join human_cust_personnel_assignment_type b on b.assignment_type = a.assignment_type
			left join human_cust_personnel_assignment_reason c on c.assignment_reason = a.assignment_reason
	...
	public function penugasan_detail_pro_add($data){
             //get last max end_date for update
                $sqla = $this->db->query ("select * from human_pa_md_emp_assignment_additional where personnel_id = '$data[personnel_id]' and end_date = (select MAX(end_date) from human_pa_md_emp_assignment_additional WHERE personnel_id = '$data[personnel_id]')");
...
remark
		...
		$sqlb = $this->db->query("insert into human_pa_md_emp_assignment_additional (personnel_id,start_date,end_date,status_process,assignment_type,assignment_reason,external_id,employee_area,employee_office,employee_type,employee_subtype,lit_code_position,unit,work_location,legal_letter_no,legal_date,legal_effective_date,remark) VALUES ('$data[personnel_id]','$data[start_date]','$data[end_date]','$data[status_process]','$data[assignment_type]','$data[assignment_reason]','$data[external_id]','$data[employee_area]','$data[employee_office]','$data[employee_type]','$data[employee_subtype]','$data[lit_code_position]','$data[unit]','$data[work_location]','$data[legal_letter_no]','$data[legal_date]','$data[legal_effective_date]','$data[remark]')");
                ...
                $sqlc =$this->db->query("update human_pa_md_emp_assignment_additional set end_date = '$data[start_date]' where personnel_id = '$personnel_idfirst' AND start_date = '$start_datefirst'");
	...
	public function penugasan_detail_update($personnel_id,$start_date,$end_date){
...
c.nama_jabatan as namajabatan,d.nm_unit as namaunit,b.code_core_orm,b.name_position,b.id_jabatan,b.tipe,case when b.tipe = 'L' then 'Laut' else 'Darat' end as statuswilayah
from human_pa_md_emp_assignment_additional a 
...
remark
              */
		$query = $this->db->query("update human_pa_md_emp_assignment_additional set status_process = '$data[status_process]', 
