...
*/
	...
	public function __construct(){
        ...
        public function  penugasan_opt_posisi(){
            $lit_code_position = $this->input->post('lit_code_position');
            echo json_encode($this->model_penugasan->penugasan_opt_posisi($lit_code_position));
		...
		$data = json_encode($this->model_penugasan->get_all_penugasan_detail_json($personnel_id));
		echo "{\"records\":" . $data . "}";
	}
	...
	public function penugasan_detail_delete($personnel_id,$start_date,$end_date){
		$personnel_id = $this->uri->segment(3);
		$start_date = $this->uri->segment(4);
		$end_date = $this->uri->segment(5);
		$sqldelete = $this->model_penugasan->penugasan_detail_delete($personnel_id,$start_date,$end_date);
	...
	public function penugasan_detail_add(){
		...
		$sql        =   $this->db->query("SELECT *,substr(birth_date,1,4) as thnlahir FROM human_pa_md_emp_personal where personnel_id = '$personnel_id' order by start_date desc limit 1")->row_array();
	 	...
	 	$data = array('judul'=>'Human Resource Information System (HRIS) ASDP',
					  ...
					  'opt_position'=>$this->model_penugasan->opt_position(),
	...
	public function penugasan_detail_update($personnel_id,$start_date,$end_date){
		...
		$sql        =   $this->db->query("SELECT *,substr(birth_date,1,4) as thnlahir FROM human_pa_md_emp_personal where personnel_id = '$personnel_id' order by start_date desc limit 1")->row_array();
 	 	...
 	 	$data_assignment = $this->model_penugasan->penugasan_detail_update($personnel_id,$start_date,$end_date);
...
remark
		...
		$data = array('personnel_id'=>$this->input->post('personnel_id'),
					 ...
					 // 'jabatan'=>$this->input->post('kdjabatan'),
					  ...
					  'legal_date'=>$tanggalsk,
				  	  'legal_effective_date'=>$tanggalberlaku,
		...
		$sql = $this->model_penugasan->penugasan_detail_pro_update($data);
				...
				window.location='".base_url()."/penugasan/penugasan_detail/".$data['personnel_id']."';
		      </script>";
