...
class Model_endorsement extends CI_Model {
/*
...
*/
	...
	//ini method untuk menarik seluruh data endorsement
	public function get_all_endorsement(){
	...
	public function get_all_endorsement_detail_json($personnel_id){
 	
	$query = $this->db->query("select *,date_format(start_date,'%d-%m-%Y') as tglmulai,date_format(end_date,'%d-%m-%Y') as tglakhir from human_pa_md_emp_endorsment where personnel_id = '$personnel_id' order by start_date desc");
			...
			$list_data = null;
	                ...
	                'opsi'=> '<a href="'.base_url('endorsement/endorsement_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'" title="Update"> </a><a href="'.base_url('endorsement/endorsement_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'" title="Hapus"></a>'
                        //'opsidelete'=> '<a href="'.base_url('endorsement/endorsement_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'"> </a>',
	                //'opsiupdate'=> '<a href="'.base_url('endorsement/endorsement_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'"></a>'
	            );
	        }
	     return $list_data;
	}

	public function endorsement_detail_delete($personnel_id,$start_date,$end_date){
		$sql = $this->db->query("delete from human_pa_md_emp_endorsment where personnel_id = '$personnel_id' AND start_date = '$start_date' AND end_date = '$end_date'");
		return $sql;
	}
	public function get_all_pic_endorsement_json($personnel_id){
	...
	$query = $this->db->query("select *,case when gender = '1' then 'Pria' else 'Wanita' end as genderstatus, date_format(start_date,'%d %M %Y') as tanggalmasuknya from human_pa_md_emp_personal where personnel_id = '$personnel_id' order by personnel_id asc");
	        ...
	        foreach ($query->result() as $row) {
	            $list_data[] = array(
	                ...
	                'tipe_endorsement'=>$row->tanggalmasuknya,
	                'alasan_endorsement' => $row->tanggalmasuknya
	            );
	        }
	     return $list_data;
	}
	...
	//ini method untuk menyimpan data endorsement baru
	public function pro_add_endorsement($nik,$stor_tanggal_masuk,$nama){
	...
	public function endorsement_detail($id){
	...
	public function endorsement_detail_pro_add($data){
                //get last max end_date for update
                $sqla = $this->db->query ("select * from human_pa_md_emp_endorsment where personnel_id = '$data[personnel_id]' and end_date = (select MAX(end_date) from human_pa_md_emp_endorsment WHERE personnel_id = '$data[personnel_id]')");
                $personnel_idfirst = '';
                $start_datefirst = '';
...
remark
                 *                  */
		$sqlb = $this->db->query("insert into human_pa_md_emp_endorsment (personnel_id,start_date,end_date,status_process,endorsment_no,authority,remark) VALUES ('$data[personnel_id]','$data[start_date]','$data[end_date]','$data[status_process]','$data[endorsment_no]','$data[authority]','$data[remark]')");
                ...
                $sqlc =$this->db->query("update human_pa_md_emp_endorsment set end_date = '$data[start_date]' where personnel_id = '$personnel_idfirst' AND start_date = '$start_datefirst'");
	...
	public function endorsement_detail_update($personnel_id,$start_date,$end_date){ 
            
		$sql = $this->db->query("select *,date_format(start_date,'%d-%m-%Y') as tanggalstart,date_format(end_date,'%d-%m-%Y') as tanggalend from human_pa_md_emp_endorsment where personnel_id = '$personnel_id' and start_date = '$start_date' and end_date = '$end_date'");
		return $sql;
	}

	public function endorsement_update_selected($id){
	...
	public function endorsement_detail_pro_update($data){
		          /*
personnel_id
start_date
end_date
...
remark
                 */
		$query = $this->db->query("update human_pa_md_emp_endorsment set status_process = '$data[status_process]', 
							endorsment_no = '$data[endorsment_no]',
	...
	public function endorsement_delete($id){
		$query = $this->db->query("delete from human_pa_md_emp_personal where personnel_id = '$id' ");
		return $query;
	}

	public function pro_update_endorsement($nik,$stor_tanggal_masuk,$nama){
	...
	public function endorsement_update_pro_selected($data){
