	        ...
	        foreach ($query->result() as $row) {
	            $list_data[] = array(
	                ...
	                'alasan_jabatan' => $row->alasan_jabatan,
                        'opsi'=> '<a href="'.base_url('riwayat_jabatan/riwayat_jabatan_detail_update').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/edit.png').'" title="Update"> </a><a  onclick="return confirm(Anda Yakin?);"  href="'.base_url('riwayat_jabatan/riwayat_jabatan_detail_delete').'/'.$row->personnel_id.'/'.$row->start_date.'/'.$row->end_date.'""> <img src="'.base_url('assets/images/delete.png').'" title="Hapus"></a>'
...
where a.lit_code_position = '$lit_code_position'");
	...
	public function riwayat_jabatan_detail($id){
	...
	public function riwayat_jabatan_detail_pro_add($data){
...
remark
		...
		$sqlb = $this->db->query("insert into human_pa_md_emp_assignment (personnel_id,start_date,end_date,status_process,assignment_type,assignment_reason,external_id,employee_area,employee_office,employee_type,employee_subtype,lit_code_position,unit,work_location,legal_letter_no,legal_date,remark) VALUES ('$data[personnel_id]','$data[start_date]','$data[end_date]','$data[status_process]','$data[assignment_type]','$data[assignment_reason]','$data[external_id]','$data[employee_area]','$data[employee_office]','$data[employee_type]','$data[employee_subtype]','$data[lit_code_position]','$data[unit]','$data[work_location]','$data[legal_letter_no]','$data[legal_date]','$data[remark]')");
...
where a.personnel_id = '$personnel_id' and a.start_date = '$start_date' and a.end_date = '$end_date'");
	...
	public function riwayat_jabatan_detail_pro_update($data){
		/* 
personnel_id
start_date
end_date
